<?php
namespace DKH\eBaySDK\Types;

use \DKH\eBaySDK\Types;
use \DKH\eBaySDK\Exceptions;
use \DKH\eBaySDK\JmesPath\Env;
use \DKH\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \DKH\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \DKH\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \DKH\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'DKH\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \DKH\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\DKH\eBaySDK\Types\BaseType', false)) {
