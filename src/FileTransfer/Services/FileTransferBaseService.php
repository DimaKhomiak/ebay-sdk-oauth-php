 ...
 */
class FileTransferBaseService extends \DKH\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \DKH\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
     ...
     */
    protected function callOperationAsync($name, \DKH\eBaySDK\Types\BaseType $request, $responseClass)
    {
        /**
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \DKH\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \DKH\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \DKH\eBaySDK\FileTransfer\Types\XopInclude([
